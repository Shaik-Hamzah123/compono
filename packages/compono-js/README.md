...
schema → resolver → validator → render pipeline, rendering via
[`pptxgenjs`](https://www.npmjs.com/package/pptxgenjs) instead of
`python-pptx`. Same primitive-JSON contract as the Python package, same
"validate → fix → render" feedback loop, independent implementation and
independent version — for agent harnesses that write/call JS rather than
Python.
...
Full v1 primitive catalog (`header`, `text`, `image`, `stat`, `grid`,
`table`, `sequence`, `chart`, `shape` including connector routing) —
same resolver algorithm (flex-equal body stacking, 2D grid math,
Liang-Barsky connector-routing with gutter-detour fallback), same
...
Not yet ported: Inspire, `review()`, DOCX generation — those stay
Python-only for now (`compono` core). No MCP server for this package yet
either.
